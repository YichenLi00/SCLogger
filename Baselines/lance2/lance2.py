...
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "" # add google credentials
BASE_DIR = ""  # add base directory of google buckets
TPU_TOPOLOGY = "2x2"
tpu = tf.distribute.cluster_resolver.TPUClusterResolver("")  # add TPU 
TPU_ADDRESS = tpu.get_master()
master_address = tpu.master()
#task_info = tpu.get_task_info()
cluster_spec = tpu.cluster_spec()
...
# add the path of vocabulary model
vocab_model_path = BASE_DIR + '' #@param { type: "string" }
vocab_path = BASE_DIR + '' #@param { type: "string" }
...
PRETRAINED_DIR=BASE_DIR + '/multiple_pred_model_t1/'#@param { type: "string" }
...
#                pretrained_model_dir=PRETRAINED_DIR)
    ...
    #checkpoint_steps=-1
    #)
dataset_list = ["log_study"]
for item in dataset_list:
    model.batch_size = 256
    input_file = BASE_DIR + f''#@param { type: "string" }
    output_file = BASE_DIR+ f''#@param { type: "string" }
